...
(async()=>{
 ...
 try{
  ...
  const creationDefaults=await page.evaluate(()=>['bot-name','bot-agent','bot-directory'].map(id=>document.getElementById(id).value));
  await page.locator('#bot-existing').click();
  for(const id of ['bot-scan','bot-link','bot-existing','bot-save','bot-app-id']) assert(await page.locator('#'+id).isVisible(),id+' missing in manual mode');
  await page.evaluate(()=>{
  ...
  assert.equal(await page.locator('#bot-save').isVisible(),false);
  ...
  await page.locator('#bot-existing').click();
  assert.equal(await page.locator('#bot-error').textContent(),'');
  assert(await page.locator('#bot-created-app').isVisible(),'keep the known app recovery link');
  ...
  for(let attempt=0;attempt<2;attempt++){
   ...
   assert(await page.locator('#bot-save').isVisible());
   assert.equal(await page.locator('#bot-link-fields').isVisible(),false);
   assert.equal(await page.locator('#bot-error').textContent(),'');
   assert.deepEqual(await page.evaluate(()=>['bot-name','bot-agent','bot-directory'].map(id=>document.getElementById(id).value)),creationDefaults);
   for(const id of ['bot-scan','bot-link','bot-existing']) assert(await page.locator('#'+id).isVisible());
   assert(await page.locator('#bot-scan').isDisabled(),'method switch must not permit duplicate creation');
   await page.locator('#bot-link').click();
   await emit({stage:'apps',apps:[{app_id:'cli_created',name:'测试2'}]});
   await page.evaluate(()=>window.botTestStream.close());
   await page.locator('#bot-link-fields').waitFor({state:'visible'});
  ...
  await page.locator('#bot-existing').click();
  await page.setViewportSize({width:390,height:844});
  assert(await page.locator('#bot-dialog').evaluate(e=>e.scrollWidth<=e.clientWidth),'manual methods overflow on mobile');
  await page.screenshot({path:'/tmp/iris-bot-manual-methods-mobile.png'});
  await page.setViewportSize({width:1440,height:1000});
